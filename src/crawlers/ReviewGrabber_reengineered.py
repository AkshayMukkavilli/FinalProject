...
import random
import re
from upper_lower_percentages import percentages_upper_lower
...
users = [
    ...
    'Mozilla/5.0 (Linux; U; Android 4.3; de-de; SAMSUNG GT-I9300/I9300XXUGNA5 Build/JSS15J) AppleWebKit/534.30 '
    '(KHTML, like Gecko) Version/4.0 Mobile Safari/534.30',
    'Mozilla/5.0 (Linux; U; Android 4.4.2; en-us; SPH-L710 Build/KOT49H) AppleWebKit/534.30 (KHTML, like Gecko) '
    'Version/4.0 Mobile Safari/534.30',
    'Mozilla/5.0 (Linux; U; Android 4.3; de-de; SAMSUNG GT-I9305/I9305XXUENH1 Build/JSS15J) AppleWebKit/534.30 '
    '(KHTML, like Gecko) Version/4.0 Mobile Safari/534.30',
...
for asin in ASIN_list:
    ...
    all_reviews_link = soup.find('a', {'class': "a-link-emphasis a-text-bold",
                                       'data-hook': "see-all-reviews-link-foot"})

    while all_reviews_link is None:
        source_code = requests.get(url, headers=header)
        plain_text = source_code.text
        soup = BeautifulSoup(plain_text, "lxml")
    ...
    while page <= iterator:
        ...
        reviews = soup_1.findAll('span', {'class': "a-size-base review-text review-text-content",
                                          'data-hook': "review-body"})
        # The type of reviews is <class 'bs4.element.ResultSet'>
        # Thus, converting it into string format below
        str_reviews = str(reviews)

        one_review_per_line = str_reviews.replace('<span class="a-size-base review-text review-text-content" '
                                                  'data-hook="review-body"><span class="">', '')
        ...
        with open(asin + 'metadata.csv', 'a') as fa:
            with open(asin + 'titles.txt', 'a', encoding="utf-8") as fs:
                for rev in soup_1.findAll('div', {'data-hook': "review", 'class': "a-section review aok-relative"}):
                    ...
                    rep = {"January": "01", "February": "02", "March": "03", "April": "04", "May": "05",
                           "June": "06", "July": "07", "August": "08", "September": "09", "October": "10",
                           "November": "11", "December": "12"}  # define desired replacements here
                    ...
                    helpful_votes = str(mini_soup.find('span', {'data-hook': "helpful-vote-statement",
                                                                'class': "a-size-base a-color-tertiary "
                                                                         "cr-vote-text cr-vote-error "
                                                                         "cr-vote-component"}))

                    helpful_votes = str(mini_soup.find('span', {'data-hook': "helpful-vote-statement",
                                                                'class': "a-size-base a-color-tertiary cr-vote-text "
                                                                         "cr-vote-error cr-vote-component"}))
                    try:
                        helpful_votes = helpful_votes.replace('<span class="a-size-base a-color-tertiary cr-vote-text '
                                                              'cr-vote-error cr-vote-component" '
                    ...
                    if helpful_votes == 'None':
                        helpful_votes = str(mini_soup.find('span', {'data-hook': "helpful-vote-statement",
                                                                    'class': "a-size-base a-color-tertiary "
                                                                             "cr-vote-text"}))
                        try:
                            helpful_votes = helpful_votes.replace('<span class="a-size-base a-color-tertiary '
                                                                  'cr-vote-text" '
                                                                  'data-hook="helpful-vote-statement">', '')
                        except:
                            pass
    ...
    with open(asin + 'titles.txt', 'r', encoding="utf-8") as title_read:
        with open(asin + 'titles_metadata.csv', 'w+') as title_write:
            for t_line in title_read.readlines():
                ...
                title_metadata = str(no_words_in_title) + ',' + str(length_title) + ',' + str(title_upper_percentage) \
                    + ',' + str(title_lower_percentage) + '\n'
